        ...
        if (mysqli_num_rows($result) == 1) {
            ...
            if ($result_fetch['verified'] == 1) {
                if($result_fetch['password']==$_POST['password']){
                    ...
                    $_SESSION['patient_email'] = $result_fetch['email'];
                    $_SESSION['patient_id'] = $result_fetch['id'];
                    $_SESSION['patient_fullname'] = $result_fetch['fullname'];
                    $_SESSION['patient_phone'] = $result_fetch['phone'];
                    setcookie("patientemail",$result_fetch['email'],time()+(86400*30),"/");
                    header("location:patient_dashboard?email=$_SESSION[patient_email]&id=$_SESSION[patient_id]");
                }else{
                    echo "
                    <script>
                ...
                <form class="p-3" action="" method="POST" autocomplete="off">
                    <div class="form-group py-2">
                        <div class="input-field">
                            <input type="email" value="<?php if(isset($_COOKIE['patientemail'])){ echo $_COOKIE['patientemail']; }  ?>" name="email" placeholder="Enter your Email" required class="form-control px-3 py-2">
                        </div>
                    </div>
                    <div class="form-group py-2">
                        <div class="input-field">
                    ...
                    <button class="btn btn-width bg-warning text-dark" name="submit" type="submit">Log in</button>
